# AI Fundamentals

A comprehensive guide to understanding how WP Agentic Admin brings AI to your browser.

## Overview

This guide covers everything from the basics of language models to advanced optimization techniques. Whether you're new to AI or want to understand the technical implementation, these docs will help you understand what's happening under the hood.

## Core Concepts

1. [What is a Language Model?](01-what-is-a-language-model.md)
2. [Running AI in the Browser](02-running-ai-in-browser.md)
3. [WebGPU: The Graphics Card for AI](03-webgpu.md)
4. [WebLLM Library](04-webllm.md)
5. [Quantization: Making Models Smaller](05-quantization.md)

## Implementation in WP Agentic Admin

6. [Models in WP Agentic Admin](06-models-in-wp-agentic-admin.md)
7. [Service Worker Persistence](07-service-worker-persistence.md)
8. [The ReAct Pattern](08-react-pattern.md)
9. [Tools & Abilities](09-tools-and-abilities.md)

## Advanced Topics

10. [Performance & Optimization](10-performance-optimization.md)
11. [Tool Selection at Scale (RLM Approach)](11-tool-selection-at-scale.md)
12. [Debugging & Development](12-debugging-development.md)

## Additional Resources

- [Architecture Overview](../ARCHITECTURE.md)
- [Creating Custom Abilities](../ABILITIES-GUIDE.md)
- [CloudFest Hackathon Roadmap](../../CLOUDFEST_HACKATHON.md)
