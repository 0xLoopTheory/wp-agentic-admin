...
class WorkflowRegistry {
	...
	register( workflow ) {
		...
		const workflowWithDefaults = {
			...
			summarize: ( results ) =>
				this.defaultSummarize( results, workflow ),
			...workflow,
			// Normalize keywords to lowercase
			keywords: ( workflow.keywords || [] ).map( ( k ) =>
				k.toLowerCase()
			),
		};
	...
	validateWorkflow( workflow ) {
		...
		workflow.steps.forEach( ( step, index ) => {
			if ( ! step.abilityId ) {
				throw new Error(
					`Workflow ${ workflow.id } step ${ index } must have an abilityId`
				);
			}
			if ( ! step.label ) {
				throw new Error(
					`Workflow ${ workflow.id } step ${ index } must have a label`
				);
			}
		} );
	}
	 ...
	 * @return {WorkflowDefinition|undefined} The workflow definition or undefined if not found.
	 */
	get( id ) {
	 ...
	 *
	 * @return {WorkflowDefinition[]} Array of all registered workflow definitions.
	 */
	getAll() {
	 ...
	 *
	 * @param {string} id - Workflow ID
	 * @return {boolean} True if workflow with given ID exists.
	 */
	has( id ) {
	...
	createFromIntents( intents, originalMessage ) {
		...
		const steps = intents.map( ( intent, index ) => ( {
			...
			label: `Step ${ index + 1 }: ${ intent.abilityId
				.split( '/' )
				.pop()
				.replace( /-/g, ' ' ) }`,
