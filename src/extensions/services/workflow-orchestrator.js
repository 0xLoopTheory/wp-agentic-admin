...
class WorkflowOrchestrator {
		...
		try {
			...
			if ( workflow.requiresConfirmation ) {
				...
				const confirmed = await this.callbacks.onConfirmationRequired(
					workflow,
					this.getConfirmationDetails( workflow )
				);
			...
			for ( let i = 0; i < workflow.steps.length; i++ ) {
				...
				if ( ! stepResult.success ) {
					...
					this.currentState.error = new Error(
						`Step ${ i + 1 } failed: ${
							stepResult.result.error || 'Unknown error'
						}`
					);
	...
	async evaluateLLMCondition(
		...
		try {
			...
			const context = this.buildContextFromResults(
				previousResults,
				initialParams
			);

			// Step 2: Interpolate variables in prompt
			const interpolatedPrompt = this.interpolatePrompt(
				promptTemplate,
				context
			);
...
Respond with JSON only: { "execute": true/false, "reason": "brief explanation" }`;
	...
	buildContextFromResults( previousResults, initialParams ) {
		...
		previousResults.forEach( ( stepResult, index ) => {
			if ( stepResult.success && stepResult.result ) {
				...
				if ( result.database_size ) {
					context.dbSize = result.database_size;
				}
				if ( result.last_optimized ) {
					context.lastOptimized = result.last_optimized;
				}
				if ( result.plugins ) {
					context.pluginCount = result.plugins.length;
				}
				if ( result.cache_size ) {
					context.cacheSize = result.cache_size;
				}
				if ( result.error_count ) {
					context.errorCount = result.error_count;
				}
	...
	getConfirmationDetails( workflow ) {
		...
		const destructiveSteps = stepsWithDetails.filter(
			( s ) => s.isDestructive
		);
	 ...
	 * @return {WorkflowResult} Result object with success status, steps, and error details.
	 */
	 ...
	 *
	 * @return {boolean} True if workflow is running or awaiting confirmation.
	 */
	isRunning() {
		return (
			this.currentState?.status === 'running' ||
	 ...
	 *
	 * @return {WorkflowState|null} Current state or null if no workflow running.
	 */
	getState() {
	 ...
	 * @return {Promise<WorkflowResult>} Result of executing the dynamically created workflow.
	 */
	async executeFromIntents( intents, originalMessage ) {
		// Create ad-hoc workflow from intents
		const workflow = workflowRegistry.createFromIntents(
			intents,
			originalMessage
		);
	 ...
	 *
	 * @return {string} User-friendly status message describing current progress.
	 */
	getProgressMessage() {
		if ( ! this.currentState ) {
